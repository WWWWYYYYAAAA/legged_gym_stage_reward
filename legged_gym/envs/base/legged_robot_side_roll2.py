...
from legged_gym.envs.base.base_task_p import BaseTask
...
class LeggedRobotBF(BaseTask):
    ...
    def compute_reward(self):
        ...
        self.rew_buf[:] += self.stage_buf[:, 1]*(-torch.abs(com_height - 0.1))
        self.rew_buf[:] += self.stage_buf[:, 2]*(-torch.clamp(com_height - 0.2, min=0.0))
        self.rew_buf[:] += self.stage_buf[:, 3]*(-torch.clamp(com_height - 0.2, min=0.0))
        ...
        self.rew_buf[:] += self.stage_buf[:, 2]*(-torch.abs(torch.arccos(torch.clamp(body_z[:, 0], -1.0, 1.0)) - np.pi/2.0))
        self.rew_buf[:] += self.stage_buf[:, 3]*(-torch.abs(torch.arccos(torch.clamp(body_z[:, 0], -1.0, 1.0)) - np.pi/2.0))
        self.rew_buf[:] += self.stage_buf[:, 4]*(-torch.arccos(torch.clamp(body_z[:, 2], -1.0, 1.0)))
        # rotation angle
        self.rew_buf[:] +=  self.stage_buf[:, 0]*(0.0)
        self.rew_buf[:] += self.stage_buf[:, 1]*(0.0)
        ...
        self.rew_buf[:] += self.stage_buf[:, 2]*(-masks*roll_angles + (1.0 - masks)*(roll_angles - 2.0*np.pi))
        roll_angles = torch.arccos(torch.clamp(body_z[:, 2], -1.0, 1.0)) # target: (0, 0, 1)
        masks = torch.logical_or(body_z[:, 1] <= 0, torch.abs(roll_angles) < np.pi/6.0).type(torch.float)
        self.rew_buf[:] += self.stage_buf[:, 3]*(-masks*roll_angles + (1.0 - masks)*(roll_angles - 2.0*np.pi))
        self.rew_buf[:] += self.stage_buf[:, 4]*(0.0)
        # base vel
        x_dirs = torch_utils.quat_rotate(self.base_quaternions, self.world_x)
        x_dirs[:, 2] = 0.0
        ...
        self.rew_buf[:] += self.stage_buf[:, 2]*((base_ang_vel_x < 4.0*np.pi).type(torch.float)*base_ang_vel_x)
        self.rew_buf[:] += self.stage_buf[:, 3]*((base_ang_vel_x < 4.0*np.pi).type(torch.float)*base_ang_vel_x)
        self.rew_buf[:] += self.stage_buf[:, 4]*(-vel_penalty)
        # energy
        self.rew_buf[:] += -torch.square(self.dof_torques).mean(dim=-1)
        # style
        self.rew_buf[:] +=  self.stage_buf[:, 0]*(-torch.square(self.dof_positions - self.default_dof_positions).mean(dim=-1))
        self.rew_buf[:] += self.stage_buf[:, 1]*(-torch.square(self.dof_positions - self.crawled_dof_positions).mean(dim=-1))
        ...
        self.rew_buf[:] += self.stage_buf[:, 2]*(-style_penalty)
        ...
        self.rew_buf[:] += self.stage_buf[:, 3]*(-style_penalty)
        self.rew_buf[:] += self.stage_buf[:, 4]*(-torch.square(self.dof_positions - self.default_dof_positions).mean(dim=-1))
        # ========================================================= #
        # ==================== calculate costs ==================== #
        # foot contact
